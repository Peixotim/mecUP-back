...
const envSchema = z
  .object({
    ...
    // JWT — segredos distintos por tipo de token; TTLs configuráveis via ENV
    JWT_ACCESS_SECRET: z.string().min(32, 'JWT_ACCESS_SECRET must be at least 32 characters'),
    JWT_REFRESH_SECRET: z.string().min(32, 'JWT_REFRESH_SECRET must be at least 32 characters'),
    JWT_ACCESS_TTL: z.string().min(1, 'JWT_ACCESS_TTL is required').default('15m'),
    JWT_REFRESH_TTL: z.string().min(1, 'JWT_REFRESH_TTL is required').default('7d'),
  })
